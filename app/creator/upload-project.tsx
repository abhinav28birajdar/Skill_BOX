...
export default function UploadProjectScreen() {
  ...
  const [projectFile, setProjectFile] = useState<DocumentPicker.DocumentPickerAsset | null>(null);
  ...
  const pickProject = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        setProjectFile(result.assets[0]);
      }
    } catch (error) {
      Alert.alert('Error', 'Failed to pick project file');
    }
  };
  ...
  const validateForm = () => {
    ...
    if (!projectFile) {
      newErrors.project = 'Please select a project file';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Upload project file
      const projectPath = await uploadFile(projectFile, 'content', 'projects');
      ...
      Alert.alert(
        'Success',
        'Project resource uploaded successfully! It will be reviewed before being published.',
        [{ text: 'OK', onPress: () => router.back() }]
      );
    } catch (error) {
      console.error('Upload error:', error);
      Alert.alert('Error', 'Failed to upload project resource. Please try again.');
    } finally {
      setUploading(false);
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.header}>
          ...
          <ThemedText style={styles.headerSubtitle}>
            Share complete projects, code examples, and starter templates
          </ThemedText>
        </View>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.formGroup}>
            ...
            <TextInput
              ...
              placeholder="Enter tags separated by commas (e.g., starter-kit, complete-project, source-code)"
            />
          </View>
          ...
          <View style={styles.formGroup}>
            ...
            >
              <Text style={styles.fileButtonText}>
                {projectFile 
                  ? `📦 ${projectFile.name} (${getFileSize(projectFile.size || 0)})` 
                  : '📦 Select Project File'}
              </Text>
            </TouchableOpacity>
            ...
            {projectFile && (
              <View style={styles.fileInfo}>
                <Text style={styles.fileInfoText}>
                  ✅ Ready to upload: {projectFile.name}
                </Text>
                <Text style={styles.fileInfoSubtext}>
                  Size: {getFileSize(projectFile.size || 0)}
                </Text>
              </View>
            )}
          </View>
          ...
          {/* Thumbnail */}
          <View style={styles.formGroup}>
            <Text style={styles.label}>Project Thumbnail (optional)</Text>
            <Text style={styles.helperText}>
              Add a preview image or screenshot of your project
            </Text>
          ...
          <View style={styles.guidelines}>
            <Text style={styles.guidelinesTitle}>📋 Project Guidelines</Text>
            <Text style={styles.guidelinesText}>
              ...
              • Test that the project runs after extraction
            </Text>
          </View>
          ...
          >
            <Text style={styles.submitButtonText}>
              {uploading ? 'Uploading...' : 'Upload Project Resource'}
            </Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
